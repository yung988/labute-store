...
export const OrderCreateSchema = z.object({
  ...
  customer_phone: z.string().regex(/^(\+420|\+421)?[0-9]{9}$/, "Invalid Czech/Slovak phone number").optional(),
...
export const ProductCreateSchema = z.object({
  ...
  slug: z.string().regex(/^[a-z0-9-]+$/, "Slug must contain only lowercase letters, numbers, and hyphens").max(100, "Slug too long"),
...
export const ProductUpdateSchema = z.object({
  ...
  slug: z.string().regex(/^[a-z0-9-]+$/, "Slug must contain only lowercase letters, numbers, and hyphens").max(100, "Slug too long").optional(),
  price_cents: z.number().int().min(0, "Price cannot be negative").max(10000000, "Price too high").optional(),
  product_images: z.array(ProductImageSchema).max(10, "Too many images").optional(),
...
export const InventoryUpdateSchema = z.object({
  sku_id: z.string().uuid("Invalid SKU ID"),
  stock_change: z.number().int().min(-1000, "Stock change too large").max(1000, "Stock change too large"),
