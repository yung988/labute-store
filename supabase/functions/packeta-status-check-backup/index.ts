...
Deno.serve(async (_req: Request) => {
  ...
  if (!SITE_URL || !CRON_SECRET) {
    const missing = [!SITE_URL && "SITE_URL", !CRON_SECRET && "CRON_SECRET"].filter(Boolean).join(", ");
    return new Response(
      JSON.stringify({ error: `Missing required env(s): ${missing}` }),
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ success: true, upstream: body }),
      { status: 200, headers: { "Content-Type": "application/json" } }
    );
  } catch (e: any) {
    return new Response(
      JSON.stringify({ error: "Edge function error", message: String(e) }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
});
