...
export async function POST(req: NextRequest) {
  ...
  if (process.env.PACKETA_API_DISABLED === 'true') {
    return NextResponse.json(
      {
        error:
          'Packeta API is temporarily disabled due to service issues. Please try again later or contact support.',
        disabled: true,
      },
      { status: 503 }
    );
  ...
  if (!isHomeDelivery && !order.packeta_point_id) {
    return NextResponse.json(
      { error: 'No Packeta point selected for this pickup order' },
      { status: 400 }
    );
  }
  ...
  if (!PACKETA_API_PASSWORD) {
    return NextResponse.json(
      {
        error:
          'Packeta API key is not configured. Please set PACKETA_API_PASSWORD environment variable.',
      },
      { status: 500 }
    );
  }
  ...
  if (!eshopId) {
    return NextResponse.json(
      {
        error:
          'Packeta eshop ID is not configured. Please set PACKETA_ESHOP_ID environment variable.',
      },
      { status: 500 }
    );
  }
  ...
  if (isHomeDelivery && order.delivery_postal_code) {
    ...
    if (!formattedPostalCode.match(/^\d{5}$/)) {
      return NextResponse.json(
        { error: 'Invalid postal code format. Must be 5 digits.' },
        { status: 400 }
      );
    }
  }
  ...
  if (isHomeDelivery) {
    if (!order.delivery_address || order.delivery_address.trim().length < 3) {
      return NextResponse.json(
        { error: 'Valid delivery address is required for home delivery' },
        { status: 400 }
      );
    }
    if (!order.delivery_city || order.delivery_city.trim().length < 2) {
      return NextResponse.json(
        { error: 'Valid delivery city is required for home delivery' },
        { status: 400 }
      );
    }
  ...
  if (!packetaResponse.ok) {
    ...
    if (packetaResponse.status === 504) {
      return NextResponse.json(
        {
          error:
            'Packeta API is temporarily unavailable (gateway timeout). Please try again in a few minutes.',
        },
        { status: 503 }
      );
