          ...
          const methods: ShippingMethod[] =
            data.shipping_methods?.map(
              (method: {
                ...
                delivery_type:
                  method.code === 'PACKETA_PICKUP'
                    ? 'pickup_point'
                    : method.code === 'PACKETA_HOME_DELIVERY'
                      ? 'home_delivery'
                      : 'unknown',
              })
            ) || [];
  ...
  const items: CartItem[] = useMemo(
    () =>
      cart?.line_items?.map((item) => ({
        ...
        size: item.variant.option_values.find((ov) => ov.name.toLowerCase().includes('size'))
          ?.presentation,
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50">
          <div className="bg-white w-[90%] max-w-sm p-6 border border-gray-300 text-center">
            ...
            <p className="text-xs text-gray-600 mb-4">
              Byli jste přesměrováni, nebo klikněte na tlačítko níže pro pokračování v nákupu.
            </p>
            <button
              type="button"
              onClick={() => router.push('/')}
            ...
            <form className="space-y-8">
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">
                      {isChangingDelivery && deliveryMethod !== 'home_delivery'
                        ? 'Načítám...'
                        : 'Doručit domů'}
                    </span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-sm font-medium">
                      {isChangingDelivery && deliveryMethod !== 'pickup'
                        ? 'Načítám...'
                        : 'Výdejní místo'}
                    </span>
                  </button>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="text-right">
                          <p className="font-medium text-xs">
                            {deliveryPrice.toLocaleString('cs-CZ')} Kč
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      onPointSelect={(point) => {
                        ...
                        if (point) {
                          const branch: SelectedBranch = {
                            ...
                            name: point.name || '',
                            address:
                              `${point.street || ''}, ${point.zip || ''} ${point.city || ''}`.trim(),
                          };
                          ...
                          <input
                            ...
                            onBlur={(e) => {
                              if (e.target.value && !validatePostalCode(e.target.value)) {
                                setValidationErrors((prev) => ({
                                  ...prev,
                                  postalCode: 'Neplatné PSČ (formát: 123 45)',
                                }));
                              }
                            }}
                            className={`w-full border p-3 text-sm focus:outline-none rounded-none ${
              ...
              <div className="pt-6">
                ...
                ((deliveryMethod === 'pickup' && selectedPickupPoint) ||
                  (deliveryMethod === 'home_delivery' &&
                    formData.address &&
                    (manualAddressMode ? formData.city && formData.postalCode : true))) &&
                  ...
                  <div>
                    <StripePaymentElement
                      billingData={{
                        ...
                        postalCode:
                          deliveryMethod === 'home_delivery' ? formData.postalCode : undefined,
                      }}
                      pickupPoint={
                        deliveryMethod === 'pickup' && selectedPickupPoint
                          ? {
                        ...
                        {Object.keys(validationErrors).length > 0
                          ...
                          : !formData.email ||
                            ...
                            : deliveryMethod === 'pickup' && !selectedPickupPoint
                              ...
                              : deliveryMethod === 'home_delivery' &&
                                  (!formData.address ||
                                    (manualAddressMode && (!formData.city || !formData.postalCode)))
